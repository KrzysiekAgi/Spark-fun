...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    val questionRdd = postings.filter(_.postingType == 1).map(p => (p.id, p))
    val answerRdd = postings.filter(p => p.postingType == 2 && p.parentId.isDefined).map(p => (p.parentId.getOrElse(-1), p))
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    grouped.map(candidate => (candidate._2.head._1, answerHighScore(candidate._2.map(_._2).toArray)))
  }
